...
def test_build_pending_scale_episodes_minimum_one(tmp_path: pathlib.Path):
    suite = _make_suite("s1")  # stage has episodes=5
    contest = _make_contest("pa")
    run_dir = _fake_run_dir({})
    steps = build_pending(suite, contest, 0.05, run_dir, retry_failed=False, record_root=tmp_path)
    assert steps[0].episodes == 1
